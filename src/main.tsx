...
function ProductPreview() {
  return <figure className="product-preview" aria-labelledby="preview-caption">
    <div className="preview-toolbar">
      <span className="preview-brand"><img src="/agentscan-icon.png" alt="" />AgentScan</span>
      <span className="preview-target">release-smoke / LangChain</span>
      <span className="example-badge">报告示例</span>
    </div>
    <div className="preview-body">
      <div className="preview-nav" aria-hidden="true">
        <span className="selected"><Code2 size={15} />概览</span>
        <span><CircleAlert size={15} />风险 <b>1</b></span>
        <span><Boxes size={15} />AgentBOM</span>
        <span><Network size={15} />关系图</span>
      </div>
      <div className="preview-report">
        <div className="preview-heading">
          <span><small>分析完成</small><strong>项目安全概览</strong></span>
          <span className="status-ready"><Check size={13} />READY</span>
        </div>
        <div className="preview-metrics">
          <span><strong>1</strong><small>Finding</small></span>
          <span><strong>2</strong><small>Components</small></span>
          <span><strong>2</strong><small>Evidence files</small></span>
        </div>
        <div className="preview-finding">
          <div className="finding-title"><span className="severity-high">HIGH</span><strong>高权限工具缺少审批</strong></div>
          <p>Agent 可以到达 shell 工具，但路径中没有确认过的人工审批控制。</p>
          <div className="finding-meta"><span>ASI-09</span><span>CWE-862</span><span><MapPin size={12} />app.py · tools.py</span></div>
          <div className="risk-path"><span>review_agent</span><ArrowRight size={14} /><span className="danger-node">shell_tool</span></div>
        </div>
      </div>
    </div>
    <figcaption id="preview-caption">基于验证契约构造的报告示例，用于说明 Finding、来源证据与风险路径之间的关系。</figcaption>
  </figure>
}
  ...
  return <article className="doc-article" id="quickstart">
    ...
    <div className="journey">
      ...
      <section className="journey-section" id="scan">
        ...
        <p className="checkpoint"><Check size={15} />完成标志：概览页出现 Finding、AgentBOM 和关系图入口。读取或解析不完整时，页面会显示明确警告。</p>
      </section>
...
function App() {
  ...
  const navigateTo = (item: Pick<SearchItem, 'page' | 'id'>) => {
    ...
    if (samePage) document.getElementById(item.id)?.scrollIntoView({ block: 'start' })
  }
